...
const AddCompanyForm = () => {
  ...
  const setOrganization = () => {
    ...
    const setOrganizationReq = async (organizationId) => {
      ...
      axios
        .post(
          `/jobcreator/request-organization`,
          { id: organizationId },
  ...
  useEffect(() => {
    if (inputValue !== '') {
      axios
        .get(`/organization/search?q=${inputValue}&limit=10&offset=0`)
        .then((response) => {
                  ...
                  <Stack direction={'row'} spacing={1}>
                    {option.displayPicture ? (
                      ...
                      <Avatar sx={{ width: 24, height: 24 }}>
                        {option.displayName &&
                          Array.from(option.displayName)[0]}
                      </Avatar>
                    )}
            ...
            <CardWithCloseButton onClose={() => setValue(null)}>
              <Stack direction={'row'} alignItems='center' spacing={2}>
                ...
                <Stack direction={'column'}>
                  <Typography variant='profile_name'>
                    {value.displayName}
                  </Typography>
          ...
          <Stack direction='row' spacing={2}>
            ...
            <Box sx={{ width: '100%' }}>
              <Button
                ...
                disabled={submitOrgLoading || value == null}
              >
                Continue
              </Button>
            </Box>
